...
class GlobalConfig(BaseModel):
    llm_api_token_env: str = "OPENAI_API_KEY" # Default env var for LLM API token
    token_size_threshold: int = 128 * 1024 # 128K tokens
    default_llm_settings: LLMSettings = Field(default_factory=LLMSettings)
    default_content_extraction_settings: ContentExtractionSettings = Field(default_factory=ContentExtractionSettings)
...
def load_global_config(path: str = GLOBAL_CONFIG_FILE) -> GlobalConfig:
    try:
        if not os.path.exists(path):
            print(f"Warning: Global config file '{path}' not found. Using default global settings.")
            return GlobalConfig()
        data = toml.load(path)
...
def load_collection(collection_path: str, global_config: GlobalConfig) -> Collection:
    ...
    try:
        ...
        overrides = CollectionOverrides(
            ...
            content_extraction_settings=collection_data.get('content_extraction_settings'),
            collection_prompt=collection_data.get('collection_prompt')
        )

        # Merge LLM settings: collection overrides global defaults
        resolved_llm_settings_data = global_config.default_llm_settings.model_dump(exclude_unset=True)
        if overrides.llm_settings:
            resolved_llm_settings_data.update(overrides.llm_settings.model_dump(exclude_unset=True))
        ...
        resolved_content_extraction_settings_data = global_config.default_content_extraction_settings.model_dump(exclude_unset=True)
        if overrides.content_extraction_settings:
            resolved_content_extraction_settings_data.update(overrides.content_extraction_settings.model_dump(exclude_unset=True))
        resolved_content_extraction_settings = ContentExtractionSettings(**resolved_content_extraction_settings_data)
        ...
        print(f"Error loading or validating collection config from {collection_path}: {e}")
        raise
